...
CREATE
OR REPLACE TEMPORARY TABLE SOB_BRANDED_ORALS AS WITH T0 AS (
    SELECT
        QTR,
        SWITCH_FROM,
        SWITCH_FROM_TREATMENT_CLASS,
...
CREATE
OR REPLACE TEMPORARY TABLE SOB_GENERIC_ORALS AS WITH T0 AS (
    SELECT
        QTR,
        SWITCH_FROM,
        SWITCH_FROM_TREATMENT_CLASS,
...
CREATE
OR REPLACE TEMPORARY TABLE SOB_LAIS AS WITH T0 AS (
    SELECT
        QTR,
        SWITCH_FROM,
        SWITCH_FROM_TREATMENT_CLASS,
...
T1 as (
    ...
    FROM
        T0
        left join "KARUNA_ANALYTICS"."REPORTING"."Master_Market_Definition_1229" B
    where
        B."Product.Name" = T0.switch_to
),
T2 as (
SELECT * FROM T1 full join Data_Dt
)
select
    *
from
