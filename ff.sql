...
with coverage as (
    select a.formulary_id, a.payer_id, a.segment_id, a.drug_id, b.drug_name, a.tier, a.status, a.nomenclature from "KARUNA"."ANCILLARY_DATA"."COVERAGE_NOMENCLATURE" 
...
all_combinations AS (
 SELECT
  ...
  0 as value
FROM
  (SELECT DISTINCT product_group FROM final) AS t1
CROSS JOIN
  (SELECT DISTINCT status FROM final) AS t2
CROSS JOIN
  (SELECT DISTINCT metric FROM final) AS t3
CROSS JOIN
  (SELECT DISTINCT segment_group_update FROM final) AS t4
CROSS JOIN
  (SELECT DISTINCT indication FROM final) AS t5
CROSS JOIN
  (SELECT DISTINCT month, month_order FROM final) AS t6
)
...
, group_common as
(
  select metric,indication, segment_group_update, status, month, month_order, product_group, sum(value) AS Value from missing_combinations
  group by 1, 2, 3, 4, 5, 6, 7
)
select * from group_common;
