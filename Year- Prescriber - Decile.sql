...
CASE
    ...
    ELSE spcl_group
END as spcl_group, class, dosage_type, brand_generic, "decile_2_and_above", provider_id, ovr_factored_trx_cnt, month_id
...
from T0
where ovr_factored_trx_cnt > 0 and (SPCL_GROUP is not null and SPCL_GROUP != '' and SPCL_GROUP != 'UNKNOWN')
...
, T18 AS
(
Select class, dosage_type, brand_generic, spcl_group, cast(month_id/100 as INT) as year, sum(ovr_factored_trx_cnt) as value1
from T0
where ovr_factored_trx_cnt > 0 and (SPCL_GROUP is not null and SPCL_GROUP != '' and SPCL_GROUP != 'UNKNOWN')
group by 1, 2, 3, 4, 5
)
...
CASE
    ...
    WHEN spcl_group = 'PEDIATRICS' THEN 'OTHERS'
    ELSE spcl_group
END as spcl_group, class, dosage_type, brand_generic, "decile_2_and_above", provider_id, ovr_factored_nbrx_cnt, month_id
FROM "KARUNA_ANALYTICS"."ADM"."PROVIDER_RX"
)
, T1 AS
(
Select class, dosage_type, brand_generic, spcl_group, provider_id, cast(month_id/100 as INT) as year
from T0
where ovr_factored_nbrx_cnt > 0 and (SPCL_GROUP is not null and SPCL_GROUP != '' and SPCL_GROUP != 'UNKNOWN')
group by 1, 2, 3, 4, 5, 6
)
-- one 'ALL' data starts
, T2 as
...
, T18 AS
(
Select class, dosage_type, brand_generic, spcl_group, cast(month_id/100 as INT) as year, sum(ovr_factored_nbrx_cnt) as value1
from T0
where ovr_factored_nbrx_cnt > 0 and (SPCL_GROUP is not null and SPCL_GROUP != '' and SPCL_GROUP != 'UNKNOWN')
group by 1, 2, 3, 4, 5
)
...
CASE
    ...
    WHEN spcl_group = 'PEDIATRICS' THEN 'OTHERS'
    ELSE spcl_group
END as spcl_group, class, dosage_type, brand_generic, "decile_2_and_above", provider_id, schiz_factored_trx_cnt, month_id
FROM "KARUNA_ANALYTICS"."ADM"."PROVIDER_RX"
)
, T1 AS
(
...
CASE
    ...
    WHEN spcl_group = 'PEDIATRICS' THEN 'OTHERS'
    ELSE spcl_group
END as spcl_group, class, dosage_type, brand_generic, "decile_2_and_above", provider_id, schz_factored_nbrx_cnt, month_id
FROM "KARUNA_ANALYTICS"."ADM"."PROVIDER_RX"
)
, T1 AS
(
Select class, dosage_type, brand_generic, spcl_group, provider_id, cast(month_id/100 as INT) as year
from T0
where schz_factored_nbrx_cnt > 0 and (SPCL_GROUP is not null and SPCL_GROUP != '' and SPCL_GROUP != 'UNKNOWN')
group by 1, 2, 3, 4, 5, 6
)
-- one 'ALL' data starts
, T2 as
...
, T18 AS
(
Select class, dosage_type, brand_generic, spcl_group, cast(month_id/100 as INT) as year, sum(schz_factored_nbrx_cnt) as value1
from T0
where schz_factored_nbrx_cnt > 0 and (SPCL_GROUP is not null and SPCL_GROUP != '' and SPCL_GROUP != 'UNKNOWN')
group by 1, 2, 3, 4, 5
)
...
Select class, dosage_type, brand_generic, spcl_group, provider_id, cast(month_id/100 as INT) as year
from T0
where ovr_factored_trx_cnt > 0 and (SPCL_GROUP is not null and SPCL_GROUP != '' and SPCL_GROUP != 'UNKNOWN') and "decile_2_and_above" = 'Y'
group by 1, 2, 3, 4, 5, 6
)
-- one 'ALL' data starts
, T2 as
...
Select class, dosage_type, brand_generic, spcl_group, provider_id, cast(month_id/100 as INT) as year
from T0
where ovr_factored_nbrx_cnt > 0 and (SPCL_GROUP is not null and SPCL_GROUP != '' and SPCL_GROUP != 'UNKNOWN') and "decile_2_and_above" = 'Y'
group by 1, 2, 3, 4, 5, 6
)
-- one 'ALL' data starts
, T2 as
...
Select class, dosage_type, brand_generic, spcl_group, provider_id, cast(month_id/100 as INT) as year
from T0
where schiz_factored_trx_cnt > 0 and (SPCL_GROUP is not null and SPCL_GROUP != '' and SPCL_GROUP != 'UNKNOWN') and "decile_2_and_above" = 'Y'
group by 1, 2, 3, 4, 5, 6
)
-- one 'ALL' data starts
, T2 as
...
Select class, dosage_type, brand_generic, spcl_group, provider_id, cast(month_id/100 as INT) as year
from T0
where schz_factored_nbrx_cnt > 0 and (SPCL_GROUP is not null and SPCL_GROUP != '' and SPCL_GROUP != 'UNKNOWN') and "decile_2_and_above" = 'Y'
group by 1, 2, 3, 4, 5, 6
)
-- one 'ALL' data starts
, T2 as
...
CREATE OR REPLACE TABLE "KARUNA_ANALYTICS"."REPORTING"."PRESCRIBER_RX_SUMMARY_YEAR_DECILE" AS
WITH T1 AS
(
